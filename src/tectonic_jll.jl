...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import tectonic_jll on Julia versions older than 1.3!")
end
    ...
    function dev_jll()
        ...
        mktempdir() do temp_env
            Pkg.activate(temp_env) do
                Pkg.develop("tectonic_jll")
            end
        end
        # Create the override directory
        override_dir = joinpath(Pkg.devdir(), "tectonic_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"tectonic", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "tectonic_jll", "src", "tectonic_jll.jl"))
        @info("tectonic_ll dev'ed out to /depot/dev/tectonic_jll with pre-populated override directory")
    end
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("d7dd28d6-a5e6-559c-9131-7eb760cdacc5"))
platforms = [Pkg.Artifacts.unpack_platform(e, "tectonic", artifacts_toml) for e in artifacts["tectonic"]]
...
if best_platform === nothing
    @debug("Unable to load tectonic; unsupported platform $(triplet(platform_key_abi()))")
else
